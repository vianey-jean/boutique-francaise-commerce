...
const ModernTable: React.FC<ModernTableProps> = ({ children, className }) => {
  return (
    <div className="rounded-lg sm:rounded-xl border-0 shadow-lg overflow-x-auto lg:overflow-x-visible bg-white dark:bg-gray-900 -mx-3 sm:mx-0">
      <Table className={cn("min-w-full lg:w-full", className)}>
        {children}
      </Table>
    </div>
  );
...
const ModernTableHead: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className }) => (
  <TableHead className={cn(
    "font-bold text-gray-700 dark:text-gray-300 bg-gradient-to-r from-transparent to-transparent text-xs sm:text-sm whitespace-nowrap lg:px-2",
    className
  )}>
    {children}
  </TableHead>
...
const ModernTableCell: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className }) => (
  <TableCell className={cn("text-gray-600 dark:text-gray-400 text-xs sm:text-sm whitespace-nowrap lg:px-2", className)}>
    {children}
  </TableCell>
);
